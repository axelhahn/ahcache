...
<head>
    ...
    <style>
        body{background:#eee; background: linear-gradient(-20deg,#eee,#fff) fixed;border-top: 4px solid #bbc;color:#333; font-family: verdana,arial; font-size: 1.0em; margin: 0em; padding: 1em;}
        button{padding: 1em; font-size: 1em; border-radius: 0.3em; border: 1px solid rgba(0,0,0,0.1); }
        a{color:#669;}
        h1{margin: 0 0 1em; border-bottom: 1px solid #ccc;}
        h1 a{color:#557;text-decoration: none;}
        h2{margin-top:0;color:#889;}
        footer{background: rgba(0,0,0,0.05); border-top-left-radius: 1em; position: fixed; bottom: 1em; padding: 1em; right: 1em;}
        nav{float: left; margin-right: 1em; min-width: 15%; border-right: 0px dashed #ccc; border-bottom: 0px dashed #ccc; box-shadow: 0.3em 0.3em 0.7em #ddd; }
        nav ul li a{padding: 0.5em;}
        pre{background: #f0f4f8;}
        table{border: 1px solid #ccc; }
        table.right{float: right; margin-left: 5em; padding: 0.2em; background: #e4e0f0; background: none; border: none;}
        table.dataTable thead{ background: #bbc;}
        td{vertical-align: top;}
        ul{list-style: none; margin: 0; padding: 0;}
        ul li a{text-decoration: none; color:#666; display:block; padding: 0;}
        ...
        #details{opacity: 0.97; overflow: scroll; position: fixed; top: 5%; left: 5%; width: 90%; height: 85%; background: #fff; padding: 1em; border: 3px solid; box-shadow: 0 0 3em #000; }
        #maincontent{float: left;  border: 0px solid; min-width:70%; max-width:90%; padding-left: 1em; margin-bottom: 8em;}
        #navigation{float: left; border: 0px solid; }

        .button{background:#ccc; color:#333; border-radius: 0.3em; border: 1px solid rgba(0,0,0,0.1); padding: 0.5em; text-decoration: none;}
        .delete{background:#e0dcdc; background:#f0ecec; border: 0px; color:#611;}
        .delete:hover{background:#ecc;}

        .bar{border: 1px solid rgba(0,0,0,0.05); display: block; float: left; width: 100px;}
        .counter{font-size: 180%; color: #bbc; padding-left: 1em;}
        .bar .left{background:#9b9; height: 1em;}
        .dataTables_wrapper{ box-shadow: 0 0 3em #ddd; padding: 1em;}
        .less30 .bar .left{background:#da8;}
        .less30,   .less30 a{background:#fec; color:#c80;}
        .ok,       .ok a{color:#080;}
        .outdated, .outdated a{background:#edd !important; color:#a66;}
        .active,   .active a {background:#556 !important;color:#fff;}
    </style>
...
<body>
    ...
    $(document).ready( function () {
        $('.datatable').DataTable( {"lengthMenu":[[10,50,100,-1],[10,50,100,"..."]], "order": [ 3, 'asc' ], stateSave: true} );
    } );
    </script>
</body>
</html>